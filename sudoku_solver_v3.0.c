 ...
 * Version 3.0 - Fixes the bug for exiting if puzzle is wrong
 */
...
/*Below method checks if some grid position is being inserted with value 
 * when the previous grid position is still empty. This shows a no soln.
 * scenario. 
 */
...
int insertValue(int row, int col,int prev_val){
    ...
    if(checkPrevEmptySlot(row,col)==1){
    	printf("Something's amiss...No Solution to this puzzle!\n\n");
    	exit(0);
    }
